...
export default function GameDetail({ gameId, onBack, isAdmin }) {
  ...
  return (
    <div className="pb-6">
      ...
      <button
        onClick={onBack}
        className="flex items-center gap-2 text-emerald-400 text-sm font-medium mb-4 active:opacity-60 transition-opacity"
      >
      ...
      <div className="relative rounded-2xl overflow-hidden mb-5">
        <div className={`absolute inset-0 ${
          g.status === "active"
            ? "bg-gradient-to-br from-red-700/40 via-orange-800/20 to-slate-900"
            : g.status === "finished"
              ? "bg-gradient-to-br from-slate-700/40 to-slate-900"
        ...
        <div className="relative p-5">
          ...
          <div className="space-y-1.5 text-[15px]">
            ...
            <p className="text-gray-400">🎯 {MODE[g.game_mode]} • {g.total_rounds} раундів</p>
          </div>
          ...
          {g.status === "active" && (
            <div className="mt-4">
              <div className="flex justify-between text-xs text-gray-400 mb-1.5">
                <span>Раунд {g.current_round}/{g.total_rounds}</span>
                <span>{Math.round((g.current_round / g.total_rounds) * 100)}%</span>
              </div>
              <div className="h-2 bg-slate-700/50 rounded-full overflow-hidden">
                <div
                  className="h-full bg-gradient-to-r from-red-500 to-orange-500 rounded-full transition-all duration-500"
                  style={{ width: `${(g.current_round / g.total_rounds) * 100}%` }}
                />
              </div>
            </div>
          )}
      ...
      {g.status !== "finished" && g.status !== "cancelled" && (
        <div className="space-y-2 mb-5">
          {!myRegistration && (
            <ActionButton
              onClick={() => doAction(() => joinGame(gameId))}
              loading={actionLoading === "joinGame"}
              icon="📝" label="Записатись на гру"
              className="bg-gradient-to-r from-emerald-600 to-teal-600"
            />
          )}
          {myRegistration?.attendance === "registered" && (g.status === "checkin" || g.status === "upcoming") && (
            <ActionButton
              onClick={() => doAction(() => checkinGame(gameId))}
              loading={actionLoading === "checkinGame"}
              icon="📍" label="Check-in — я на місці"
              className="bg-gradient-to-r from-amber-600 to-orange-600"
            />
          )}
          {g.status === "active" && myRegistration?.attendance === "checked_in" && (
            <ActionButton
              onClick={() => doAction(() => reportDead(gameId))}
              loading={actionLoading === "reportDead"}
              icon="💀" label="Мене вбили"
      ...
      {g.status === "active" && round?.active && (
        <div className="bg-red-950/15 border border-red-800/30 rounded-2xl p-4 mb-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              onClick={load}
              className="text-xs text-gray-500 bg-slate-800/60 px-3 py-1.5 rounded-lg active:scale-95 transition-transform"
            >
              🔄 Оновити
            </button>
          </div>
                      ...
                      <div
                        ...
                        className={`flex items-center justify-between p-2.5 rounded-xl transition-all ${
                          p.is_alive
                            ...
                            : "bg-slate-800/30 border border-slate-800/30 opacity-50"
                        }`}
                      >
                        <div className="flex items-center gap-2.5">
                          ...
                          <div>
                            ...
                            {p.kills > 0 && (
                              <span className="text-xs text-emerald-400 ml-2 font-bold">{p.kills}K</span>
                            )}
                          </div>
                        </div>
                        {isAdmin && p.is_alive && (
          ...
          {/* End round */}
          {isAdmin && g.game_mode !== "ffa" && (
            <div className="mt-4 pt-3 border-t border-red-900/30">
              <p className="text-xs text-gray-500 mb-2 font-medium">Хто виграв раунд?</p>
              <div className="flex gap-2">
                <button
                  onClick={() => doAction(() => adminEndRound(gameId, "A"))}
                ...
                <button
                  onClick={() => doAction(() => adminEndRound(gameId, "B"))}
...
function StatusBadge({ status }) {
  ...
  return (
    <span className={`px-3 py-1 rounded-xl text-[11px] font-bold border ${config[status] || ""}`}>
...
function ActionButton({ onClick, loading, icon, label, className }) {
  return (
    <button
      onClick={onClick}
      disabled={loading}
      className={`w-full flex items-center justify-center gap-2 py-4 rounded-2xl font-bold text-[15px] shadow-lg transition-all active:scale-[0.98] disabled:opacity-50 ${className}`}
    >
      {loading ? (
...
function SmallButton({ onClick, icon, label, color }) {
  const colors = {
    amber: "bg-amber-700/40 border-amber-600/30 text-amber-300",
    red: "bg-red-700/40 border-red-600/30 text-red-300",
    emerald: "bg-emerald-700/40 border-emerald-600/30 text-emerald-300",
  };
  return (
    <button
      onClick={onClick}
      className={`flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-sm font-bold border active:scale-95 transition-transform ${colors[color] || colors.emerald}`}
    >
